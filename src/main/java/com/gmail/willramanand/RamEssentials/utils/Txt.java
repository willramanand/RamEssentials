...
public class Txt {
    ...
    public static String parse(Lang message) {
        String stringMessage = RamEssentials.getInstance().getLang(message);

        return parse(stringMessage);
    }

    public static String process(Lang message, String... replacements) {
        return replace(parse(message), replacements);
    }
